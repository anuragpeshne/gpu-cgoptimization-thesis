...
on CPU, it is necessary to minimize the time spent on block transfers. This has been
...
returns a block of equal or greater size. Hence, the size of the requested block cannot
...
copy operation on \texttt{Block} object is initiated. At the same time, a bit
...
along with MPI to send an unpinned memory buffer, in all there will be 3 memory copy
