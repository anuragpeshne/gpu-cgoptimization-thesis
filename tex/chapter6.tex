...
the first request to the server takes longer as number of index range increases. This
...
for the first request to the server. Thus to compensate for the high cost of first
...
\texttt{wait\_time\_} as \texttt{wait\_time\_} for first request grows and the
...
decreases. This is clear from figure~\ref{fig:prefetch_real}, as the difference
in run time of programs without prefetch and prefetch increase. It is worth noticing
that the only couple of programs show significant increase in difference while one
of the programs shows no difference in prefetching and no prefetching. To study
why this is the case, total \texttt{wait\_time} of the programs is plotted in
...
for block to be transfered over the network. The program \texttt{sch\_rhf} did not
...
Even though there is not much improvement in speed if address of GPU memory is
