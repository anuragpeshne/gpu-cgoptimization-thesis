...
3-5 times more expensive than \texttt{malloc}. To overcome this, a scheme was
...
is done in solving the problems of efficiently allocation memory dynamically such
...
keeping. One of it, \texttt{fig:free\_list}, presented in figure~\ref{fig:free_list},
maps the block size to a list of \texttt{double} pointers. When a request for
...
sizes are pinned and returned to OS and then a new block is requested. If there
are no blocks in \texttt{free\_list} and block size is greater than free memory
than system is out of memory and exception is thrown.
