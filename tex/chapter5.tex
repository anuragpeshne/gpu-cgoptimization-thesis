...
in the order in which they are issued by the hode code~\cite{overlapdatatransfer}.
...
\subsubsection{Non Blocking Device Synchronization}
GPU streams were used to implement non blocking SIA blocks synchronization. Current
generation of GPU have dual DMA controllers~\cite{teslaspecs}\cite{teslakspecs}
and one kernel execution engine. These hardware facilities have been exploited by
implementing asynchronous memory copying for block synchronization.

To implement the non blocking synchronization, the module
\texttt{gpu\_super\_instructions} which
