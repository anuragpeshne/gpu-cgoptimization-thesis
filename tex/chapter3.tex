...
instructions to operate on them. This is different from conventional programming
languages where in computation is described in terms of individual floating point
number and operations which work on these individual numbers. For better performance
and higher utlization of resources these individual numbers should be aggregated.
This adds considerable complexity to programs since now the programmer has be careful
of the index arithmetic. Algorithms in SIA can be directly expressed in terms of
blocks. This has multiple performance advantages: moving data in terms
of blocks is more efficient since it will have less overhead per individual number
and since transfering these blocks will take significant time, runtime can overlap
computation and network operation.
...
instructions are called as super instructions.
