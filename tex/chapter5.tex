...
The first optimization includes saving unnecessary block synchronizations.
...
The memory allocated on CPU or main memory is pageable by default. GPUs cannot
access data on such pageable host memory~\cite{datatransferoptimization}
\cite{programmingguidecuda} directly. For this reason, when a memory copy operation is requested
from a host to a GPU, the CUDA driver first allocates a temporary \textbf{page locked},
or \textit{pinned}, memory on the host (main memory), copies the host memory to the
...
that, using this, GPU can read main memory at PCI-E speeds.
...
developed to reuse the pinned memory pages.
...
allocation of memory, which is handled by Operating System (OS). Extensive work~\cite{Wilson1995}
...
\subsubsection{Implementation of Caching Mechanism}\label{sec:cachingmech}
...
sizes are unpinned and returned to OS. After there is enough free memory, a new,
...
Temporary blocks in SIAL are deleted at end of each iteration in a loop. Page
locked memory allocated to temporary blocks can be reused very efficiently.
Effective caching mechanism is implemented by allocating page locked memory
...
To support non blocking synchronization, during runtime, multiple streams are created and
stored in the module \texttt{gpu\_super\_instructions}. This is the same module
...
multiple blocks. Servers distribute these blocks to workers and workers return
...
to be transfered to another node from GPU memory, it has to be copied to 3 different
...
7 copy operations, including 3 similar operations on other side, and a network
...
However, servers in SIA are not allocated GPU since servers are not responsible
